...
- **Token 配额**: 管理员可以为团队或用户设置基于 Token 数量的消费配额（例如，每月 1M 免费 Tokens，或分配给团队的可用额度）。当配额耗尽时，系统可配置为拒绝服务或发送告警。
...
| **US-004** | 作为一名**平台管理员**，我想要为不同的团队设置每月 Token 消耗上限，以避免资源超支。             | AC-004.1: 团队管理页面有输入框可以设置月度 Token 配额。AC-004.2: 当团队消耗达到配额的90%时，系统应向管理员发送告警。AC-004.3: 当团队消耗超出配额时，该团队所有成员的API请求将被拒绝。                                                          |
...
| **免费模型 Token 额度耗尽** | 虽然模型免费，但若用户/团队分配的“免费 Token 额度”耗尽，Gateway 应返回 `429 Too Many Requests` 或自定义错误码。 |
