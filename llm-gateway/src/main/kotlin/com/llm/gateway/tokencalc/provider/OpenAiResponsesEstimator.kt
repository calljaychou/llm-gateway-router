...
@Component
class OpenAiResponsesEstimator : AbstractTokenProviderEstimator() {

    override val protocol: TokenProtocol = TokenProtocol.OPENAI_RESPONSES

    override val providerName: String = "OpenAI Responses"

    override fun buildPromptText(params: TokenEstimateParams): TokenTextExtractResult {
        return TokenTextBuilder.buildOpenAiResponsesPromptText(params.requestBody)
    }

    override fun buildCompletionText(params: TokenEstimateParams): TokenTextExtractResult {
        return TokenTextBuilder.buildOpenAiResponsesCompletionText(params.responseBody, params.stream)
    }

    /** 构造 OpenAI Responses 默认 Token 明细。 */
    ...
    /** 提取 OpenAI Responses usage，兼容 input/output 与 prompt/completion 两套字段。 */
    override fun extractUsageFromObject(root: JSONObject): TokenUsageSummaryDto {
        ...
        return TokenUsageSummaryDto(
            inputTokens = usage.getIntValue("input_tokens").takeIf { it > 0 }
                ?: usage.getIntValue("prompt_tokens"),
            outputTokens = usage.getIntValue("output_tokens").takeIf { it > 0 }
                ?: usage.getIntValue("completion_tokens"),
            totalTokens = usage.getIntValue("total_tokens"),
        )
    }

    /** 流式 response.completed 事件可能将 usage 放在 response 对象内。 */
    override fun extractUsageFromStreamEvent(event: JSONObject): TokenUsageSummaryDto {
        ...
        return event.getJSONObject("response")?.let { extractUsageFromObject(it) } ?: TokenUsageSummaryDto()
    }
